...
- 🧬 **Автоматическая кластеризация** по смыслу (SemanticClusterer)
- 🔗 **Cosine similarity** вместо эвристических правил
...
### 5. Semantic Embeddings (v1.2) ⭐

Векторное представление намерений для глубокого понимания смысла:

```rust
use soma_cognitive::{IntentEmbeddings, cosine_similarity};

let embeddings = IntentEmbeddings::new();

// Получить embedding-векторы
...
// Вычислить semantic similarity
let similarity = cosine_similarity(&stabilize_emb, &healing_emb);
// => 0.979 (97.9% близости)
```

**Улучшения v1.2:**
- **Точность**: 78.3% overlap vs 42% в v1.1 (+86.5%)
